...
function ChartTooltip({ active, payload, label, prefix = '', suffix = '' }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ color: '#8B95B0', marginBottom: 4 }}>{label}</p>
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color || '#00A651', fontWeight: 700 }}>
...
function StatCard({ label, value, sub, color = '#00A651' }) {
  return (
    <div
    ...
    >
      <div className="absolute top-0 left-0 right-0" style={{ height: 2, backgroundColor: color, opacity: 0.9 }} />
      <p className="text-xs font-semibold uppercase tracking-widest mb-1 mt-0.5" style={{ color: '#4A5470' }}>{label}</p>
      <p
        className="text-2xl font-black leading-tight tabular-nums"
        style={{ color, fontFamily: 'Syne, system-ui, sans-serif' }}
      >
        {value}
      </p>
      {sub && <p className="text-xs mt-0.5" style={{ color: '#4A5470' }}>{sub}</p>}
    </div>
  );
}
    ...
    <div className="flex flex-col gap-0 pb-24">
      ...
      <div className="px-4 pt-6 pb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: '#4A5470' }}>Recharge history · Cost insights</p>
        </div>
        <button
          onClick={() => setShowForm(f => !f)}
          className="btn-press flex items-center gap-2 px-4 py-2.5 rounded-btn text-sm font-black btn-glow"
      ...
      {showForm && (
        <div className="px-4 pb-4">
          <form
            ...
            style={{ backgroundColor: '#111827', border: '1px solid rgba(0,166,81,0.25)', boxShadow: '0 0 0 1px rgba(0,166,81,0.08)' }}
          >
            <p
              className="text-sm font-black text-textPrimary"
              style={{ fontFamily: 'Syne, system-ui, sans-serif' }}
            >
              Log a Recharge
            </p>
            ...
            {formError && (
              <div className="text-xs px-3 py-2 rounded-btn" style={{ backgroundColor: 'rgba(229,57,53,0.1)', color: '#fca5a5', border: '1px solid rgba(229,57,53,0.25)' }}>
                {formError}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A5470' }}>Date</label>
                <input type="date" value={formDate} onChange={e => setFormDate(e.target.value)} style={inputSty} required />
              </div>
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A5470' }}>Amount (₦)</label>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A5470' }}>Units (kWh)</label>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A5470' }}>Token No.</label>
                <input type="text" placeholder="Optional" value={formToken} onChange={e => setFormToken(e.target.value)} style={inputSty} />
              </div>
            </div>

            {liveCostPerKwh && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-btn text-xs font-semibold" style={{ backgroundColor: 'rgba(0,166,81,0.08)', color: '#00A651' }}>
                ⚡ {liveCostPerKwh}
              </div>
            )}

            <div>
              <label className="block text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A5470' }}>Notes</label>
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={formSaving}
                className="btn-press flex-1 py-2.5 rounded-btn text-sm font-black disabled:opacity-50 btn-glow"
                style={{ background: 'linear-gradient(180deg, #00A651 0%, #008f47 100%)', color: '#001a0f' }}
              >
              ...
              <button
                ...
                style={{ backgroundColor: '#0B0F1A', color: '#8B95B0', border: '1px solid rgba(255,255,255,0.1)' }}
              >
                Cancel
              </button>
            </div>
        ...
        <>
              ...
              <div className="rounded-card p-4" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p
                    ...
                    style={{ color: insights.estDaysRemaining <= 3 ? '#E53935' : '#00A651', fontFamily: 'Syne, system-ui, sans-serif' }}
                  >
                ...
                {(() => {
                  ...
                  const barColor = pct > 50 ? '#00A651' : pct > 20 ? '#F5A623' : '#E53935';
                  return (
                ...
                <p className="text-xs mt-1.5" style={{ color: '#4A5470' }}>
          ...
          {insights.monthlyData.length >= 2 && (
            <div className="px-4 pb-3">
              ...
              <div className="rounded-card p-3" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={insights.monthlyData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    <XAxis dataKey="label" tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} width={52}
                      tickFormatter={v => `₦${(v / 1000).toFixed(0)}k`} />
                    <Tooltip content={<ChartTooltip prefix="₦" />} />
                    <Bar dataKey="totalAmount" radius={[4, 4, 0, 0]} maxBarSize={44}>
                      {insights.monthlyData.map((_, i) => (
                        <Cell key={i} fill={i === insights.monthlyData.length - 1 ? '#00A651' : 'rgba(0,166,81,0.2)'} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              <div className="rounded-card p-3" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                <ResponsiveContainer width="100%" height={120}>
                  <BarChart data={insights.kwhChartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    <XAxis dataKey="label" tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} width={34}
                      tickFormatter={v => `${v}`} />
              ...
              <div className="rounded-card p-3" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                <ResponsiveContainer width="100%" height={120}>
                  <LineChart data={insights.costPerKwhTrend} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="label" tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} width={42}
                      tickFormatter={v => `₦${v}`} />
                    <Tooltip content={<ChartTooltip prefix="₦" suffix="/kWh" />} />
                    <Line type="monotone" dataKey="costPerKwh" stroke="#F5A623" strokeWidth={2} dot={{ fill: '#F5A623', r: 3 }} activeDot={{ r: 5 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div className="rounded-card p-3" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                <ResponsiveContainer width="100%" height={120}>
                  <BarChart data={insights.daysBetweenChartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    <XAxis dataKey="label" tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: '#4A5470', fontSize: 10 }} axisLine={false} tickLine={false} width={24}
                      tickFormatter={v => `${v}`} />
          ...
          {insights.habitsInsights.length > 0 && (
            <div className="px-4 pb-3">
              <div
                ...
                style={{ backgroundColor: '#111827', borderLeft: '3px solid #60A5FA', boxShadow: '0 0 0 1px rgba(96,165,250,0.12)' }}
              >
                <p
                  className="text-xs font-black uppercase tracking-widest mb-2"
                  style={{ color: '#60A5FA', fontFamily: 'Syne, system-ui, sans-serif' }}
                >
                  Insights
                </p>
      ...
      <div className="px-4 pb-3">
        ...
        {loading ? (
          <div className="rounded-card p-6 text-center" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
            <p className="text-sm" style={{ color: '#4A5470' }}>Loading…</p>
          </div>
        ) : recharges.length === 0 ? (
          <div className="rounded-card p-8 text-center" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <>
            <div className="rounded-card overflow-hidden" style={{ backgroundColor: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
              {pageRecharges.map((r, i) => {
                ...
                if (isEditing) {
                  return (
                    <div key={r.id} className="p-4 flex flex-col gap-2" style={{ borderBottom: borderStyle, backgroundColor: '#0B0F1A' }}>
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-xs font-semibold mb-1" style={{ color: '#4A5470' }}>Date</label>
                        ...
                        <div>
                          <label className="block text-xs font-semibold mb-1" style={{ color: '#4A5470' }}>Amount (₦)</label>
                        ...
                        <div>
                          <label className="block text-xs font-semibold mb-1" style={{ color: '#4A5470' }}>kWh</label>
                        ...
                        <div>
                          <label className="block text-xs font-semibold mb-1" style={{ color: '#4A5470' }}>Token No.</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold mb-1" style={{ color: '#4A5470' }}>Notes</label>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEditSave(r.id)} className="btn-press flex-1 py-2 rounded-btn text-xs font-black btn-glow" style={{ background: 'linear-gradient(180deg, #00A651 0%, #008f47 100%)', color: '#001a0f' }}>Save</button>
                        <button onClick={() => setEditingId(null)} className="btn-press px-3 py-2 rounded-btn text-xs font-semibold" style={{ backgroundColor: '#111827', color: '#8B95B0', border: '1px solid rgba(255,255,255,0.07)' }}>Cancel</button>
                      </div>
                    </div>
                  );
                }
                  ...
                  <div key={r.id} className="px-4 py-3 flex items-start gap-3" style={{ borderBottom: borderStyle }}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p
                          ...
                          style={{ fontFamily: 'Syne, system-ui, sans-serif' }}
                        >
                          {fmtNaira(r.amount_naira)}
                        </p>
                        {r.kwh && <p className="text-xs font-semibold" style={{ color: '#60A5FA' }}>{fmtKWh(r.kwh)}</p>}
                        {costPerKwh && <p className="text-xs" style={{ color: '#4A5470' }}>₦{costPerKwh}/kWh</p>}
                      </div>
                      <p className="text-xs mt-0.5" style={{ color: '#4A5470' }}>
                        ...
                        {daysUntil !== undefined && (
                          <span style={{ color: '#8B95B0' }}> · lasted {daysUntil}d</span>
                        )}
                      </p>
                      {r.token_number && (
                        <p className="text-xs mt-0.5 font-mono tracking-wider" style={{ color: '#8B95B0' }}>{r.token_number}</p>
                      )}
                      {r.notes && <p className="text-xs mt-0.5" style={{ color: '#4A5470' }}>{r.notes}</p>}
                    </div>
                    <div className="flex gap-1 shrink-0">
                      <button
                        ...
                        style={{ backgroundColor: 'rgba(255,255,255,0.06)', color: '#8B95B0' }}
                        title="Edit"
                      >✏</button>
                      <button
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 mt-3">
                <button
                  ...
                  style={{ backgroundColor: '#111827', color: '#8B95B0', border: '1px solid rgba(255,255,255,0.07)' }}
                >← Prev</button>
                <span className="text-xs" style={{ color: '#4A5470' }}>{page + 1} / {totalPages}</span>
                <button
